...
export default function Login() {
    ...
    useEffect(() => {
      ...
      setNotAllow(true);
    }, [emailValid, pwValid, phoneVaild]);

    const handleEmail = (e) => {
      setEmail(e.target.value);
      const regex =
        /^(([^<>()\[\].,;:\s@"]+(\.[^<>()\[\].,;:\s@"]+)*)|(".+"))@(([^<>()[\].,;:\s@"]+\.)+[^<>()[\].,;:\s@"]{2,})$/i;
      if (regex.test(e.target.value) && email !== setEmail(e.target.value)) {
        setEmailValid(true);
      } else {
        setEmailValid(false);
      }
    ...
    const handlePw = (e) => {
      ...
      if (regex.test(e.target.value) && pw !== setPw(e.target.value)) {
        setPwValid(true);
      } else {
        setPwValid(false);
      }
    ...
    const handlePhone = (e) =>{
      ...
      if (phone.length === 10 && phone !== setPhone(e.target.value)) {
        setPhoneVaild(true);
      } else {
        setPhoneVaild(false);
      }
